...
import com.sy.play.enume.ErrorEnume;
import com.sy.play.enume.PalyStateEnume;
import com.sy.play.enume.PlayDirectionEnume;
import com.sy.play.util.LogUtility;
import com.sy.play.util.MeasureHelper;
import com.sy.play.util.ObjectUtil;
import com.sy.play.util.PlayUtil;
import com.sy.play.util.StringUtils;
import com.sy.play.view.FloatingWindow;
import com.sy.play.view.SYTouchSpeedLayout;
...
import static com.sy.play.enume.PalyStateEnume.STATE_PLAYING;

public abstract class SYplay extends FrameLayout implements SeekBar.OnSeekBarChangeListener, View.OnTouchListener {

    private static final String TAG = "SYplay";
    public SYTextureView textureView;
    public SYDataSource syDataSource;
    private ViewGroup textureViewContainer;
    private SYMediaInterface mediaInterface;
    ...
    public SYplay(@NonNull Context context) {
        this(context, null);
    }

    public SYplay(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SYplay(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setUp(SYDataSource syDataSource, Class kernelClass) {
        this.syDataSource = syDataSource;
    ...
    public boolean isValidDataSources() {
        return ObjectUtil.notNull(syDataSource) && ObjectUtil.notNull(syDataSource.getCurrentUrl());
    }
...
//                        }
    ...
    private VideoSniffer.SnifferListener snifferListener = new VideoSniffer.SnifferListener() {
        ...
        public void onCaptureKey(@NonNull String url, @Nullable String title, @Nullable Map<String, String> hashMap) {
            LogUtility.d("嗅探", url);
            post(() -> {
                if (syDataSource != null) {
                    syDataSource.setUrl(url);
                    syDataSource.setTitle(title);

                        syDataSource.setHeaderMap(hashMap);
    ...
    public void startVideo() {
        if (ObjectUtil.notNull(syDataSource) && ObjectUtil.isNull(syDataSource.getCurrentUrl())) {
            //这是需要进行嗅探的
            if (syDataSource.sniffing) {
                VideoSniffer.getInstance(snifferListener)
                        .addKeyWord(".m3u8")
                        .addKeyWord(".mp4")
                        .addKeyWord(".flv")
                        .parseUrl(syDataSource.sniffingUrl, syDataSource.timeOut);
                return;
            }
        }
        ...
        try {
            ...
            //内核优化
            Constructor<SYMediaInterface> constructor = kernelClass.getConstructor(SYplay.class);
    ...
    public void stopPlay() {
        ...
        if (mediaInterface != null) {
            try {
                PlayUtil.saveProgress(getContext(), syDataSource.getCurrentUrl(),
                        0, 1);
    ...
    private void addTextureView() {
        ...
        if (textureView == null) {
            textureView = new SYTextureView(getContext(), this);
        }
    ...
    public SYDataSource getSyDataSource() {
        return syDataSource;
    }
    ...
    public void continueLastTime() {
        if (mediaInterface != null) {
            long saveProgress = PlayUtil.getSavedProgress(getContext(), syDataSource.getCurrentUrl());
    ...
    public void onCompletion() {
        ...
        if (mediaInterface != null) {
            PlayUtil.saveProgress(getContext(), syDataSource.getCurrentUrl(), 0, 1);
        }
        //关闭屏幕常亮
        PlayUtil.scanForActivity(getContext()).getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        startControlViewTimer();
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (getDuration() > 0 && (mediaInterface != null && !mediaInterface.liveStreaming)) {
            ...
            PlayUtil.saveProgress(getContext(), syDataSource.getCurrentUrl(), Math.max(time, 0), mediaInterface.getDuration());
        }
        startProgressTimer();
    }
    ...
    private void onTouchDistribute(View v, MotionEvent event) {
        int id = v.getId();
        if (id == R.id.surface_container || (v instanceof SYTextureView)) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    if (!isSpeedLayoutVisual()
                            && getZoomSaveScale() == SYTextureView.MIN_SCALE
        ...
        if (id == R.id.surface_container || (v instanceof SYTextureView)) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (!isSpeedLayoutVisual() && !isSmallWindow() && !doubleClick && getZoomSaveScale() == SYTextureView.MIN_SCALE) {
                        //执行单指操作
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (mChangePosition) {
                        mChangePosition = false;
                        if (getZoomSaveScale() == SYTextureView.MIN_SCALE && mediaInterface != null && mediaInterface.isCanSeekTo()) {
    ...
    public float getZoomSaveScale() {
        if (textureView == null) {
            return SYTextureView.MIN_SCALE;
        }
    ...
    protected PlayClickGestureDetector playClickGestureDetector = new PlayClickGestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            if (e.getPointerCount() == 1) {
                if (getZoomSaveScale() == SYTextureView.MIN_SCALE) {
