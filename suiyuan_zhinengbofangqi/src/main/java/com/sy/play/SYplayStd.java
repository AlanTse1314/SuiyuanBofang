...
import com.sy.play.enume.WindowEnume;
import com.sy.play.util.LoadingSpeed;
import com.sy.play.util.LogUtility;
import com.sy.play.util.PlayUtil;
import com.sy.play.util.ScreenProjectionUtil;
import com.sy.play.util.SelectorFactory;
import com.sy.play.util.StringUtils;
import com.sy.play.view.BottomMain;
import com.sy.play.view.ButtomDialogView;
import com.sy.play.view.LoadingBarView;
import com.sy.play.view.SYTouchSpeedLayout;
import com.sy.play.view.TopMain;
import com.sy.play.view.UIOnClickListener;
import com.yanbo.lib_screen.entity.ClingDevice;

import java.util.List;

public class SYplayStd extends SYplay {
    private static final String TAG = "SYplayStd";
    ...
    public SYplayStd(@NonNull Context context) {
        this(context, null);
    }

    public SYplayStd(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SYplayStd(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setUp(SYDataSource syDataSource, Class kernelClass) {
        super.setUp(syDataSource, kernelClass);
        topMain.setTitle(syDataSource.title);
    }
    ...
    public void startScreenProjection() {
        ...
        if (clingDevice == null) {
            ...
            pausePlay();
            //设置投屏数据
            ScreenProjectionUtil.startScreenProjection(clingDevice, syDataSource.title, syDataSource.getCurrentUrl());
            //开始播放
    ...
    public void showDeviceDialog() {
        if (dialogView == null) {
            if (listAdapter == null) {
                listAdapter = new ArrayAdapter(mainActivity.getContext(), R.layout.sy_toup_item);
            }
            View view = View.inflate(mainActivity.getContext(), R.layout.sy_toup_device_layout, null);
    ...
    public void showVolumeDialog(int volumePercent) {
        if (mVolumeDialog == null) {
            mVolumeDialog = LayoutInflater.from(getContext()).inflate(R.layout.sy_dialog_volume, null);
        ...
        if (volumePercent <= 0) {
            volumeImage.setImageResource(R.drawable.sy_volume_0);
        } else if (volumePercent >= 30 && volumePercent < 60) {
            volumeImage.setImageResource(R.drawable.sy_volume_1);
        } else if (volumePercent >= 60 && volumePercent < 90) {
            volumeImage.setImageResource(R.drawable.sy_volume_2);
        } else if (volumePercent >= 90) {
            volumeImage.setImageResource(R.drawable.sy_volume_3);
        }
    ...
    public void showBrightnessDialog(int brightnessPercent) {
        if (mBrightnessDialog == null) {
            mBrightnessDialog = LayoutInflater.from(getContext()).inflate(R.layout.sy_brightness_dialog, null);
    ...
    public void showProgressDialog(int progressPercentage, long time, String currentTime, String totalTime) {
        if (mProgresstnessDialog == null) {
            mProgresstnessDialog = LayoutInflater.from(getContext()).inflate(R.layout.sy_progress_dialog, null);
